...
public class ViewUserUpdate {
	...
	private ViewUserUpdate() {
        ...
        // Username, new addition. Implements the use of UserNameRecognizer, to reject usernames that are alphanumeric, and send an error using Alert. 
    	...
    	result.ifPresent(newUsername -> {String validationError = UserNameRecognizer.checkForValidUserName(newUsername);
    		if (validationError.isEmpty()) {if (!theDatabase.doesUserExist(newUsername) || newUsername.equals(theUser.getUserName())) {
    			String oldUsername = theUser.getUserName();theDatabase.updateUsername(oldUsername, newUsername);
    			theDatabase.getUserAccountDetails(newUsername);String updatedUsername = theDatabase.getCurrentUsername();
    			theUser.setUserName(updatedUsername);if (updatedUsername == null || updatedUsername.length() < 1)label_CurrentUsername.setText("<none>");
    			else label_CurrentUsername.setText(updatedUsername); } else { Alert alert = new Alert(Alert.AlertType.ERROR);
    			alert.setTitle("Error"); alert.setHeaderText("Username '" + newUsername + "' already exists."); alert.showAndWait();}
