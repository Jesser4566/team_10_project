...
public class UserNameRecognizer {
	...
	/**********************************************************************************************
	 *Modified to work as per FSM requirements, accepting only alphabets as first characters, and size limit increased.
	 */
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				// The execution of this state is finished
				// If the size is larger than 32, the loop must stop (changed from 16)
				if (userNameSize > 32)
					running = false;
				break;			
		...
		switch (state) {
		case 0:
			// State 0 is not a final state, so we can return a very specific error message, omit 0-9
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				// There are characters remaining in the input, so the input is not valid, omit 0-9
				userNameRecognizerErrorMessage += 
					"A UserName character may only contain the characters A-Z, a-z.\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
		...
		case 2:
			// State 2 is not a final state, so we can return a very specific error message, omit 0-9
			userNameRecognizerErrorMessage +=
				"A UserName character after a period must be A-Z, a-z.\n";
			return userNameRecognizerErrorMessage;
